...
void MultiSet::serialize(const char* filename) const {
    ...
    file.write(reinterpret_cast<const char*>(&m_k), sizeof(m_k));
    file.write(reinterpret_cast<const char*>(&m_dataSize), sizeof(m_dataSize));
    file.write(reinterpret_cast<const char*>(m_data), m_dataSize);
}
...
MultiSet MultiSet::deserialize(const char* filename) {
    ...
    MultiSet ms(n, k);
    file.read(reinterpret_cast<char*>(ms.m_data), dataSize);
    return ms;
}
